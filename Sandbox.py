...
def compute_enemy_i_can_kill(result_name:str, input_file_name_str):
    heroes_db = Core.CSV.load_Heroes("InputData/HeroesDB.csv")
    my_heroes = Core.CSV.load_heroe_ids("InputData/{0}.csv".format(input_file_name_str))
    heroes_for_compute: [Core.Hero.Hero] = Core.Hero.get_all_heroes_by_hero_id(my_heroes, heroes_db)
...
def compute_enemy_i_can_kill_same_levels(result_name:str, type_hero:int, level:int):
    heroes_db = Core.CSV.load_Heroes("InputData/HeroesDB.csv")
    my_heroes: [Core.Hero.Hero] = [_ for _ in heroes_db if _.type == type_hero and _.level == level]

    result = Core.FightSimulator.fight_enemies_can_kill(my_heroes,heroes_db)
...
if __name__ == "__main__":
    ...
    #compute_enemy_i_can_kill("01","MyHandCards")
    #compute_enemy_i_can_kill("02","MyHandCardsLevel")

    #compute_enemy_i_can_kill_same_levels("SameLevel_Common",Core.Hero.CONST_Type_Common, 9)
    #compute_enemy_i_can_kill_same_levels("SameLevel_Rare", Core.Hero.CONST_Type_Rare, 7)
    #compute_enemy_i_can_kill_same_levels("SameLevel_epic", Core.Hero.CONST_Type_Epic, 5)
